...
class PQtree(object):
    ...
    def __init__(self, universe, create_root=False):
        ...
        # List of pertinent nodes to speedup reseting
        self.pertinent_nodes = []

        # Reference to pertinent root when pseudo node is created, but root is an empty node
        self.pseudo_pertinent_root = None

    def pre_reset(self):
        #for node in self.pertinent_nodes:
        #    node.reset_non_req()
    ...
    def get_pertinent_root(self, subset):
        ...
        if successor.label == Label.PARTIAL:
            if len(successor.full_children) == 1:
                return prev
            return successor
    ...
    def template_l1(self, node: PQnode) -> bool:
        if node.node_type == Type.LEAF and node.data is not None:
            node.mark_full()
            self.pertinent_nodes.append(node)
            return True
    ...
    def template_p2(self, node: PQnode) -> bool:
        ...
        if len(node.full_children) >= 2:
            ...
            full_node.mark_full()
            self.pertinent_nodes.append(full_node)
    ...
    def template_p4(self, node: PQnode) -> bool:
        ...
        if len(node.full_children) > 0:
            ...
            self.pertinent_nodes.append(new_full_node)
    ...
    def template_p6(self, node: PQnode) -> bool:
        ...
        if len(node.circular_link) == 1:
            if node.parent is None:
                ...
                self.pertinent_nodes.append(new_node)
    ...
    def template_q2(self, node: PQnode) -> bool:
        ...
        if endmost_partial_node:
            ...
            for fc in endmost_partial_node.full_children:
                fc.parent = node

            for ec in node.endmost_children:
                ec.parent = node
...
def __bubble(tree, subset):
    ...
    while QUEUE.size() + BLOCK_COUNT + OFF_THE_TOP > 1:
        ...
        # Remember pertinent node, so we can restore it later
        # tree.pertinent_nodes.append(node)
        ...
        # Mark all endmost children or children of P-node
        for i in range(2):
            if node.immediate_sublings[i] is None:
                node.mark = Mark.UNBLOCKED
    ...
    if BLOCK_COUNT == 1 and BLOCKED_NODES > 1:
        ...
        count = 0
        # tree.pertinent_root = blocked_nodes[0].parent
